...
export function CustomPackEditorWorkspace({
  ...
  const validation = useMemo(() => {
    ...
    if (total !== packSize) messages.push(`Die Packzusammensetzung enthält ${total} statt ${packSize} Karten.`);
    if (missingPools.length) messages.push(`Ohne Kartenpool: ${missingPools.join(", ")}.`);
    if (unreachablePools.length) messages.push(`Nicht über einen Packplatz erreichbar: ${unreachablePools.join(", ")}.`);
    if (slots.some((slot) => slot.rarityOptions.length === 0)) messages.push("Jeder Packplatz benötigt mindestens eine Seltenheit.");
  ...
  function resetEraPreset() {
    ...
    if (!window.confirm(`Packzusammensetzung auf „${ERA_LABELS[selected.era as CustomPackEra]}“ zurücksetzen?`)) return;
          ...
          <Panel kicker="Packinhalt" title="Seltenheitspools" className={classes("custom-pack-panel order-2 !rounded-[9px] !p-3 xl:flex xl:min-h-0 xl:flex-col", mobileView !== "POOL" && "!hidden xl:!flex")}>
            <div className="flex min-h-0 flex-1 flex-col gap-2">
              <div className="flex flex-wrap items-center justify-between gap-2 rounded-[7px] border border-white/8 bg-white/[0.02] p-2.5">
                <div><div className="flex items-center gap-2 text-xs font-semibold text-[#d9ccbc]">Packzusammensetzung <FieldHelp label="Packzusammensetzung">Packplätze bestimmen, wie viele Karten gezogen werden und welche Seltenheiten dort mit welcher Chance möglich sind. Die darunterliegenden Pools bestimmen anschließend die konkrete Karte.</FieldHelp></div><p className="mt-1 text-[0.68rem] text-[#84939b]">{slots.reduce((sum, slot) => sum + slot.count, 0)} von {packSize} Karten konfiguriert</p></div>
              ...
              <div className="grid gap-2 lg:grid-cols-2">
                {slots.map((slot, index) => (
                  <div key={slot.slotIndex} className="rounded-[8px] border border-white/8 bg-black/20 p-2.5">
                    <div className="flex items-center justify-between gap-2"><label className="text-xs font-semibold text-[#acb9bf]">Packplatz {index + 1} · Anzahl <input className="ui-input ml-2 inline-block h-8 w-16 py-1" type="number" min={1} max={100} value={slot.count} disabled={!canMutate} onChange={(event) => setSlots((current) => current.map((entry) => entry.slotIndex === slot.slotIndex ? { ...entry, count: Math.max(1, Number(event.target.value) || 1) } : entry))} /></label>{canMutate && slots.length > 1 ? <button type="button" aria-label={`Packplatz ${index + 1} entfernen`} className="text-[#c88e84]" onClick={() => setSlots((current) => current.filter((entry) => entry.slotIndex !== slot.slotIndex))}><IconX size={16} /></button> : null}</div>
                  ...
                  {rarities.map((rarity) => {
                    ...
                    return (
                      <section key={rarity} onDragOver={(event) => { event.preventDefault(); dragTargetRarityRef.current = rarity; event.dataTransfer.dropEffect = "move"; }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null) && dragTargetRarityRef.current === rarity) dragTargetRarityRef.current = null; }} onDrop={(event) => handleDrop(event, rarity)} onClick={() => setActiveRarity(rarity)} className={classes("min-h-36 rounded-[9px] border p-2.5 transition", activeRarity === rarity ? "border-teal-300/30 bg-teal-300/[0.055]" : "border-white/8 bg-white/[0.018]", missing && "border-red-300/25")}>
                        <div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2"><RarityTone rarity={rarity} /><FieldHelp label={`${rarity}-Pool`}>Alle Karten in diesem Bereich können gezogen werden, wenn ein Packplatz die Seltenheit {rarity} auswählt.</FieldHelp></div><span className="text-[0.65rem] tabular-nums text-[#7f8e95]">{entries.length} Karten</span></div>
        ...
        <div className="fixed inset-0 z-[100] grid place-items-center bg-black/70 px-4 backdrop-blur-sm">
          <div role="dialog" aria-modal="true" aria-labelledby="create-pack-title" className="w-full max-w-md rounded-[11px] border border-white/12 bg-[#0b1117] p-5 shadow-2xl">
            ...
            <div className="mt-4 grid gap-3">
              ...
              <label className="text-sm font-semibold"><span className="flex items-center gap-1">Era-Vorlage <FieldHelp label="Era-Vorlage">Lädt eine historische TCG-nahe Startverteilung. Alle Packplätze und Chancen bleiben danach anpassbar.</FieldHelp></span><select className="ui-input mt-1.5" value={newEra} onChange={(event) => setNewEra(event.target.value as CustomPackEra)}>{Object.entries(ERA_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
            </div>
